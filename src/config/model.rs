...
impl Config {
    ...
    pub fn validate(&self) -> crate::Result<()> {
        ...
        if !(10..=5_000).contains(&self.completion.local_timeout_ms)
            || (self.completion.max_candidates != 0
...
pub struct CompletionConfig {
    /// Provider work between incremental batches; later sources still run.
    pub local_timeout_ms: u64,
    /// Maximum ranked rows, or zero to keep all matching candidates.
